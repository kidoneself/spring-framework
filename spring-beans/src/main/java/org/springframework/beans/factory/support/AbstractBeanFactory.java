...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	/**
	 * Map from bean name to merged（合并） RootBeanDefinition.
	 */
	...
	protected <T> T doGetBean(
		...
		//急切地检查单例缓存中手动注册的单例。此处就是spring缓存吧
	...
	public boolean isFactoryBean(String name) throws NoSuchBeanDefinitionException {
		String beanName = transformedBeanName(name);
		/**
		 * 获取这是去三级缓存获取bean，初始化的时候，先去第三级缓存即已经初始化完成的bean集合中获取，如果获取到，下面则返回该初始化完成的bean对象·
		 */
		Object beanInstance = getSingleton(beanName, false);
		if (beanInstance != null) {
		...
		// No singleton instance found -> check bean definition.
		if (!containsBeanDefinition(beanName) && getParentBeanFactory() instanceof ConfigurableBeanFactory) {
			// No bean definition found in this factory -> delegate（委派） to parent.
